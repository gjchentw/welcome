...
### 版本遷移說明 (v1.1.0+)
如果您是從舊版本（名稱為 `Wellcome` 的時期）升級，插件在啟動時會自動偵測舊有的 `/plugins/Wellcome/` 目錄，並將其更名為 `/plugins/Welcome/`。您不需要手動移動任何設定檔。
...
# 使用語系 (en_US, zh_TW, zh_CN, ja_JP, la_US)
language: en_US
# 自動補全設定
autocomplete:
  max-players: 100
...
#### `messages.yml` (已遷移至 `lang/` 資料夾)
現在訊息由 `lang/` 下的各語系檔案管理。
...
| `/welcome reload` | `welcome.admin` | 重載設定與語系檔 |
